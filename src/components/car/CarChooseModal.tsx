...
export default function CarChooseModal() {
	...
	// Last car stuff
	const lastCars =
		useSelector((state: IState) => state.shopNew.lastCars) || [];
	useEffect(() => {
		if (Array.isArray(lastCars)) {
			if (lastCars.length <= 5) {
				if (
					!lastCars.find((car: ICar) => car.slug === currentCar!.slug)
				) {
					...
					<Fade {...TransitionProps} timeout={350}>
						<div className={classes.paper}>
							<Grid className={classes.formContainer} container>
								...
								<Grid
									className={classes.carsListContainer}
									item
									xs={12}
								>
									<Typography
										className={classes.myCarsText}
										variant="body2"
									>
